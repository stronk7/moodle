...
class core_completionlib_testcase extends advanced_testcase {
    ...
    public function test_update_state() {
        ...
        // TODO: remove or amend this, because it has no sense.
        //$c->expects($this->once())
        //    ->method('get_data')
        //    ->with($cm, false, 100)
        //    ->will($this->returnValue($current));
    ...
    public function test_get_data_no_current_user() {
        global $DB;
        $this->mock_setup();
        ...
        // TODO: Confirm if this is really correct, as part of MDL-46960, we moved, for this
        // case, from a call to get_records_sql() to a call to get_record(), and the test
        // continued passing without problem. Now, with the upgrade to phpunit9 and ->at()
        // becoming deprecated... the problem is obvious. So I've commented @ https://bit.ly/37Dl3SO
        // and have adapted the test assuming that the answer is A), aka, the code is correct
        // and the test now has zero calls to get_records_sql() and 2 calls to get_record().
        // If, finally, the answer is B) and the code is incorrect, I'll change this back to
        // the call to get_records_sql() and different returned data.
        $cache->purge();
    ...
    public function test_get_data_current_user() {
        global $DB;
        $this->mock_setup();
        ...
        // 1. Current user, single record, not from cache.
        $sillyrecord = (object)array('frog'=>'kermit');
        ...
        // 2. Current user, 'whole course', but from cache.
        $result = $c->get_data($cm, true);
        $this->assertEquals($sillyrecord, $result);

        // 3. Current user, 'whole course' and record not in cache.
        $cache->purge();
